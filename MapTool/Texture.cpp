...
CTexture::CTexture(LPDIRECT3DDEVICE9 pDevice, ETextureType eType, const TCHAR* pFilePath, _size iCount)
	: CComponent(pDevice)
...
CTexture * CTexture::Create(LPDIRECT3DDEVICE9 pDevice, ETextureType eType, const TCHAR * pFilePath, _size iCount)
{
	CTexture* pInstance = new CTexture(pDevice, eType, pFilePath, iCount);
	if (FAILED(pInstance->Ready_Component_Prototype()))
	{
		MSG_BOX(L"Failed To Create CTexture");
...
CComponent * CTexture::Clone(void * pArg)
{
	...
	{
		MSG_BOX(L"Failed To Clone CTexture");
		Safe_Release(pClone);
	}
